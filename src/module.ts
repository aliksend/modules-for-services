...
export class Module<ConfigSchema extends z.ZodTypeAny, StartArg, Value> {
  ...
  static async startAll (config: unknown, kind?: 'input' | 'output'): Promise<void> {
    for (const module of this.modules(kind)) {
      const moduleConfig = getModuleConfig(module.name, config)
...
function getModuleConfig (moduleName: string, rawFullConfig: unknown): { config: object, moduleName: string } {
  const moduleConfigPath = moduleName.split('/')

  let config: any = rawFullConfig
  while (true) {
    if (config == null) {
      break
    }
    const p = moduleConfigPath.shift()
    if (p == null) {
      break
    }
    config = config[p]
 ...
 */
function getRuntimeDirname (): string {
